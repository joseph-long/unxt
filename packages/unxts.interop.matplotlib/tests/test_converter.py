...
def test_convert_scalar_zero_d_quantity():
    """A 0-d quantity must not be iterated (regression)."""
    converter = uimpl.UnxtConverter()
    q = u.Q([1.0, 2.0], "km")[0]  # 0-d quantity
    assert converter.convert(q, u.unit("m"), axis=None) == 1000.0
